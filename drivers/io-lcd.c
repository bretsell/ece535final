...
__STATIC_INLINE void lcd_write_cmd_u8(uint8_t DL)
{
  ...
  // printf("Command IN");
  // You could choose to read the chip select and then set the CS bit to 0 using: 
  // PORT_IO_LCD_CMD->OUT &= ~MASK_IO_LCD_CSX; (without comments)
  
  // However this is inefficient to have to go and read the entire thing every time
  // Instead what we can do is use a specific IO pin that will just change the individual bit
  // OUT_CLR clears any bit that has a 1 that it is assigned to and anything with a 0 is unchanged
  PORT_IO_LCD_CMD->OUT_CLR = MASK_IO_LCD_CSX; // the mask is 1 only at bit 0, so only bit 0 is cleared/changed.
